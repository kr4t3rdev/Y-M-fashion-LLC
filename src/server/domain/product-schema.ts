...
export const productInputSchema = z
  .object({
    ...
    wholesaleUnitName: optionalString,
    wholesaleUnitQuantity: z.coerce
      .number()
      .int("Debe ser entero")
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.isWholesale && (data.wholesaleUnitName === null || data.wholesaleUnitName === undefined)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["wholesaleUnitName"],
        message: "Indica el nombre de la unidad de venta (ej. caja, docena, pack)",
      });
    }
  });
